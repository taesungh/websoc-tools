...
def _pull_page(url: str) -> str:
    ...
    try:
        ...
        raw_text = response.read().decode(encoding = 'utf-8')
...
def _parse_data(raw_page: str) -> [{str: str}]:
    # raw_listings = []
...
def get_data(query_params: {str, str}) -> [{str: str}]:
    try:
        query_url = _build_query_url(query_params)
        raw_page = _pull_page(query_url)
        course_data = _parse_data(raw_page)
        return course_data
